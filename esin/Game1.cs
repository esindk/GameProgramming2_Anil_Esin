...
public class Game1 : Game
{
    ...
    private bool _previousFlipPressed;

    private List<Rectangle> _platforms = null!;
    ...
    protected override void Initialize()
    {
        ...
        {
            new Rectangle(0, 540, 800, 60),
            new Rectangle(0, 0, 800, 40),
            new Rectangle(150, 420, 180, 20),
            new Rectangle(450, 360, 180, 20),
            new Rectangle(260, 220, 280, 20),
            new Rectangle(100, 140, 180, 20),
            new Rectangle(520, 140, 160, 20)
        };
    ...
    protected override void LoadContent()
    {
        ...
        // Content içindeki .spritefont dosyasını yükle
        try
        {
        ...
        {
            // Eğer font yüklenmezse, null kalır
            _gameFont = null;
        }
    }
    ...
    private void UpdateMenu(MouseState mouse)
    {
        ...
        // Check for clicks (only on button press, not release)
    ...
    private void UpdateGame(GameTime gameTime, KeyboardState keyboard)
    {
        ...
        bool flipPressed = keyboard.IsKeyDown(Keys.Space);
        if (flipPressed && !_previousFlipPressed)
        {
            _gravityDirection *= -1f;
        ...
        _previousFlipPressed = flipPressed;

        _playerVelocity.X = move;
        _playerVelocity.Y += GravityStrength * _gravityDirection * dt;

        Rectangle playerHorizontal = new Rectangle((int)(_playerPosition.X + _playerVelocity.X * dt), (int)_playerPosition.Y, PlayerWidth, PlayerHeight);
        Rectangle playerVertical = new Rectangle((int)_playerPosition.X, (int)(_playerPosition.Y + _playerVelocity.Y * dt), PlayerWidth, PlayerHeight);

        foreach (var platform in _platforms)
        {
            if (playerHorizontal.Intersects(platform))
            {
                ...
                _playerVelocity.X = 0;
                playerHorizontal.X = (int)_playerPosition.X;
            }
        }
        ...
        foreach (var platform in _platforms)
        {
            if (playerVertical.Intersects(platform))
            {
                if (_gravityDirection > 0 && _playerVelocity.Y > 0)
                {
                    _playerPosition.Y = platform.Top - PlayerHeight;
                    _playerVelocity.Y = 0;
                    _isGrounded = true;
                }
                else if (_gravityDirection < 0 && _playerVelocity.Y < 0)
                {
                ...
                playerVertical.Y = (int)_playerPosition.Y;
            }
        }
        ...
        if (_playerPosition.X < 0)
            _playerPosition.X = 0;
        if (_playerPosition.X > _graphics.PreferredBackBufferWidth - PlayerWidth)
            _playerPosition.X = _graphics.PreferredBackBufferWidth - PlayerWidth;
        if (_playerPosition.Y < 0)
            _playerPosition.Y = 0;
        if (_playerPosition.Y > _graphics.PreferredBackBufferHeight - PlayerHeight)
            _playerPosition.Y = _graphics.PreferredBackBufferHeight - PlayerHeight;

        if (_isGrounded && keyboard.IsKeyDown(Keys.W) || _isGrounded && keyboard.IsKeyDown(Keys.Up))
        {
            _playerVelocity.Y = -JumpSpeed * _gravityDirection;
            _isGrounded = false;
        }
    }
    ...
    private void DrawGame()
    {
        ...
        foreach (var platform in _platforms)
            DrawRectangle(platform, new Color(100, 180, 200));

        DrawRectangle(new Rectangle((int)_playerPosition.X, (int)_playerPosition.Y, PlayerWidth, PlayerHeight), new Color(235, 130, 70));

        // Draw help text
        DrawSimpleText("A/D: Move | Space: Flip | ESC: Menu", 10, 10, Color.White);
    }
    ...
    private void DrawButton(Button button, Color color)
    {
        ...
        // Gerçek metni butonun ortasına yazdırır
        if (_gameFont != null)
        {
        ...
        {
            // Fallback: text yerine basit rectangle göster
